...
@section('breadcrumb')
    ...
    <a class="breadcrumb-item active" href="{{ route('job.create') }}">@lang('global.add')</a>
@endsection
...
@section('content')
    <div class="br-section-wrapper">
        ...
        {{-- Bg content --}}
        <form action="{{ route('job.store') }}" method="post" enctype="multipart/form-data" id="frm_create_job">
            @csrf
...
@section('script')
    <script>
        $('#frm_create_job').on('submit', function (event) {
            event.preventDefault();

            var form = $('#frm_create_job');
            ...
            createJob(form.serialize());
        });

        $('#frm_create_job').validate({
            errorElement: "span",
            rules: {
                name: {
                    required: true
